...
public class SummaryBuilder implements Callable<Boolean>, SystemConstant {
    ...
    public SummaryBuilder(String workDir, String filename, int sentCountInClust, Map<String, Double> idfValues, String question, EhCacheUtil ehCacheUtil, float alpha, float beta) {
        super();
    ...
    public Boolean call() throws Exception {
        ...
        while (isNotEmpty && summaryWordCount < MAX_SUMMARY_WORDS_COUNT) {
            ...
            for (Entry<String, ClustItem> entry : candidateSentences.entrySet()) {
                ...
                for (int i = 0; i < pairs.size(); i++) {
                    ...
                    for (Word questionWord : questionWords) {
                        ...
                        // 计算当前词的tf-idf值
                        double tf = (frequencyInSummary + frequencyInSentence) / (double) (summaryWordCount + questionWords.size());
                    ...
                    // 当前句子的向量
                    double[] sentenceVec = this.sentenceToVector(words);

                    if (null != sentenceVec) {
                        // 计算当前clust的多样性得分
                        /*
                         * List<Pair<Float, String>> selectedSentencesInClust =
                         * partialSummary.get(entry.getKey());
                         * if(CollectionUtils.isNotEmpty(
                         * selectedSentencesInClust)) { for (Pair<Float, String>
                         * pairInClust : selectedSentencesInClust) { //
                         * 计算当前句子与所属clust里面已经选择的句子的相似度之和 // 计算历史句子的向量 double[]
                         * otherSentVec =
                         * this.sentenceToVector(StanfordNLPTools.segmentWord(
                         * pairInClust.getRight())); if (null == otherSentVec) {
                         * continue; } currentSentenceDiversityScore +=
                         * this.cosineDistence(sentenceVec, otherSentVec); } }
                         */
                    ...
                    // diversityScore = 1/(similarityScore + 1);
            ...
            log.info("topic name:" + this.topicname + ",\t summary words:" + summaryWordCount);
    ...
    /*public static void main(String[] args) {
        ...
        SummaryBuilder summaryBuilder = new SummaryBuilder("E:/dev_workspace/tmp/workspace/duc2007", "D0714D.txt", 10, "duc2007.idf", question, ehCacheUtil, 1.0f, 1.0f);
