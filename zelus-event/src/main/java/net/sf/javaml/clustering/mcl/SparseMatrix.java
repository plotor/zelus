...
public class SparseMatrix extends ArrayList<SparseVector> {
    ...
    public double[][] getDense() {
        ...
        for (int i = 0; i < this.size(); i++) {
            ...
            for (int j : this.get(i).keySet()) {
                aa[i][j] = this.get(i).get(j);
            }
        }
        return aa;
    }
    ...
    public void adjustMaxIndex(int i, int j) {
        ...
        if (j >= maxVLength) {
            ...
            for (int row = 0; row < this.size(); row++) {
                this.get(row).setLength(maxVLength);
            }
        }
    }
...
//        addAll(Collections.nCopies(i - size() + 1, new SparseVector()));
     ...
     */
    public int[] getSize() {
        return new int[] {this.size(), maxVLength};
    ...
    public void normaliseCols() {
        ...
        for (int row = 0; row < this.size(); row++) {
            for (int col = 0; col < this.get(row).getLength(); col++) {
                sums[col] += this.get(row).get(col);
            }
        }
        for (int row = 0; row < this.size(); row++) {
            for (int col = 0; col < this.get(row).getLength(); col++) {
                this.get(row).mult(col, 1 / sums[col]);
            }
        }
    }
    ...
    public SparseVector vectorTimes(SparseVector v) {
        ...
        for (int i : v.keySet()) {
            SparseVector a = this.get(i).copy();
            a.factor(v.get(i));
            w.add(a);
        }
        return w;
    ...
    public void hadamardProduct(SparseMatrix m) {
        for (int i = 0; i < this.size(); i++) {
            this.get(i).hadamardProduct(m.get(i));
        }
    }
    ...
    public void round(int decimalPlaces) {
        for (int i = 0; i < this.size(); i++) {
            this.get(i).round(decimalPlaces);
        }
    }
